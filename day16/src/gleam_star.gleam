...
pub fn a_star(
  ...
  do_a_star(open_set, f_score, g_score, goal, dict.new(), obstacles, #(-1, -1))
}
...
fn handle_neighbors_of_current(
  ...
  case neighbors {
    [neighbor, ..rest] -> {
      ...
      let turn_modifier = case dict.get(came_from, current) {
        Error(_) -> 0.0
        Ok(prev) -> {
          let prev: #(Int, Int) = prev
          let current: #(Int, Int) = current
          let diff_1 = #(current.0 - prev.0, current.1 - prev.1)
          let diff_2 = #(neighbor.0 - current.0, neighbor.1 - current.1)
    ...
    [] ->
      do_a_star(open_set, f_score, g_score, goal, came_from, obstacles, current)
  }
}
