...
fn get_neighbors_of_current(current: #(Int, Int)) {
  ...
  |> list.filter(fn(neighbor) {
    neighbor.0 >= 0 && neighbor.1 >= 0 && neighbor.1 <= 70 && neighbor.0 <= 70
  })
}

fn heuristic(neighbor: #(Int, Int), goal: #(Int, Int)) -> Float {
  {
    { int.power(goal.0 - neighbor.0, 2.0) |> result.unwrap(0.0) }
    +. { int.power(goal.1 - neighbor.1, 2.0) |> result.unwrap(0.0) }
  }
  |> float.square_root
  |> result.unwrap(0.0)
}
